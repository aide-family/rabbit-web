          ...
          return {
            label: (
              <Space direction='horizontal'>
                <Button size='small' shape='circle' type='link' icon={icon} />
                {label}
              </Space>
            ),
            value: +key,
...
export const getColumnList = (
  props: NotifyTemplateColumnProps
): ColumnsType<SendTemplateItem> => {
  const { onHandleMenuOnClick, current, pageSize } = props
  const tableOperationItems = (
    record: SendTemplateItem
  ): MoreMenuProps['items'] => [
      ...
      render: (record: SendTemplateItem) => (
        <Space size={20}>
          <Button
            size='small'
            type='link'
            onClick={() => onHandleMenuOnClick(record, ActionKey.DETAIL)}
          >
            详情
          </Button>
          {tableOperationItems && tableOperationItems?.length > 0 && (
            <MoreMenu
              ...
              onClick={(key: string) => {
                onHandleMenuOnClick(record, key as ActionKey)
              }}
            />
          )}
        </Space>
