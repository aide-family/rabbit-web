...
import { namespaceService, emailService, webhookService, templateService, messageLogService, healthService } from '../api/services';
import { MessageLogItem, HealthCheckReply, Status, MessageStatus } from '../api/types';
...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      const [namespaceRes, emailRes, webhookRes, templateRes, logsRes, healthRes] = await Promise.all([
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'type',
      width: 80,
      render: (type: number) => type === 0 ? <Tag color="blue">邮件</Tag> : <Tag color="green">Webhook</Tag>,
    },
    {
      title: '状态',
      dataIndex: 'status',
    ...
    <div>
      ...
      {health && (
        <Alert
          ...
          description={
            <div>
              <div><strong>状态:</strong> {health.status}</div>
              <div><strong>消息:</strong> {health.message}</div>
              <div><strong>时间:</strong> {health.timestamp}</div>
            </div>
          }
